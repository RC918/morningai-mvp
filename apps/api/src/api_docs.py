...
class Logout(Resource):
    ...
    @api.marshal_with(api.model('LogoutResponse', {
        'message': fields.String(description='登出訊息')
    }), code=200)
    @api.marshal_with(error_model, code=401)
    def post(self):
        """用戶登出"""
...
class Profile(Resource):
    @api.doc('get_profile')
    @api.doc(security='Bearer')
    @api.marshal_with(api.model('ProfileResponse', {
        'user': fields.Nested(user_model, description='用戶資訊')
    }), code=200)
    @api.marshal_with(error_model, code=401)
    def get(self):
...
class AdminUsers(Resource):
    @api.doc('list_users')
    @api.doc(security='Bearer')
    @api.marshal_with(api.model('UsersResponse', {
