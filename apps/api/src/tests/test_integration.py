...
class IntegrationTest(unittest.TestCase):
    ...
    def test_full_flow(self):
        ...
        with app.app_context():
            # 1. 註冊新使用者
            register_response = self.app.post("/api/register", json={
                "username": "testuser",
            ...
            # 3. 建立租戶
            create_tenant_response = self.app.post("/api/tenants", headers={
            ...
            # 4. 邀請新使用者
            invite_response = self.app.post(f"/api/tenants/{tenant_id}/invite", headers={
                "Authorization": f"Bearer {access_token}"
            }, json={
            ...
            # 5. 註冊被邀請的使用者
            register_invited_response = self.app.post("/api/register", json={
            ...
            # 6. 登入被邀請的使用者
            login_invited_response = self.app.post("/api/login", json={
                "email": "invited@example.com",
                "password": "password123"
            })
            ...
            # 7. 接受邀請
            accept_response = self.app.post(f"/api/tenants/invitations/{invitation_token}/accept", headers={
                "Authorization": f"Bearer {invited_access_token}"
            })
            self.assertEqual(accept_response.status_code, 200)

            # 8. 驗證成員已加入租戶
            members_response = self.app.get(f"/api/tenants/{tenant_id}/members", headers={
                "Authorization": f"Bearer {access_token}"
            })
