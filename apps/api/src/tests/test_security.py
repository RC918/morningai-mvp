...
class SecurityTestCase(unittest.TestCase):
    ...
    def test_audit_log_login(self):
        with app.app_context():
            user = User(
                username="testuser", email="test@example.com", password_hash="test"
            )
            db.session.add(user)
            db.session.commit()

            AuditLog.log_action(
                action=AuditActions.LOGIN, user_id=user.id, status="success"
            )

            log = AuditLog.query.filter_by(
                user_id=user.id, action=AuditActions.LOGIN
            ).first()
...
if __name__ == "__main__":
    ...
    def test_email_verification(self):
        with app.app_context():
            ...
            send_res = self.app.post(
                "/api/auth/email/send-verification", headers=headers
            )
            ...
            from src.routes.email_verification import (
            ...
            self.assertIn(
                "Email verified successfully", verify_res.get_json()["message"]
            )
    ...
    def test_jwt_blacklist_logout_all(self):
        with app.app_context():
            ...
            login_res = self.app.post(
                "/api/login",
                json={"email": "blacklist@example.com", "password": "password123"},
            )
            self.assertEqual(login_res.status_code, 200)
