...
class WebhookDelivery(db.Model):
    ...
    def can_retry(self):
        ...
        return (
            ...
            and (self.next_retry_at is None or self.next_retry_at <= datetime.utcnow())
        )

    def mark_as_success(
        self, response_status_code, response_headers=None, response_body=None
    ):
        """標記為成功"""
        self.status = "success"
    ...
    def schedule_retry(self, delay_seconds=None):
        ...
        if delay_seconds is None:
            delay_seconds = self.webhook.retry_delay * (
                2 ** (self.attempt_count - 1)
            )  # 指數退避
