...
def create_tenant():
    ...
    try:
        ...
        return jsonify({"message": "租戶建立成功", "tenant": tenant.to_dict()}), 201
...
def update_tenant(tenant_id):
    ...
    if not (
        user.is_admin()
        or (user.tenant_id == tenant_id and user.tenant_role in ["owner", "admin"])
    ):
        return jsonify({"message": "權限不足"}), 403
    ...
    try:
        ...
        return jsonify({"message": "租戶更新成功", "tenant": tenant.to_dict()}), 200
...
def invite_user_to_tenant(tenant_id):
    ...
    existing_invitation = TenantInvitation.query.filter_by(
        tenant_id=tenant_id, email=email, is_accepted=False, is_expired=False
    ).first()
    ...
    try:
        ...
        return (
            jsonify({"message": "邀請已發送", "invitation": invitation.to_dict()}),
            201,
        )
...
def accept_invitation(token):
    ...
    try:
        ...
        return (
            jsonify({"message": "邀請接受成功", "tenant": invitation.tenant.to_dict()}),
            200,
        )
...
def update_tenant_member(tenant_id, user_id):
    ...
    if not (
        ...
        and current_user.tenant_role in ["owner", "admin"]
    ):
        return jsonify({"message": "權限不足"}), 403
