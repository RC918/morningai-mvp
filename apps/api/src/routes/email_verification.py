...
def confirm_verification_token(token, expiration=3600):
    ...
    try:
        email = serializer.loads(
            token, salt=current_app.config.get("SECURITY_PASSWORD_SALT", "email-confirm"), max_age=expiration
        )
        return email
...
def send_verification_email(current_user):
    ...
    # 發送驗證郵件
    verification_url = f"https://morningai-mvp.onrender.com/api/auth/email/verify/{token}"
